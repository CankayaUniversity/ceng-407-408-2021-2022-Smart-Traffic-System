...
def count_vehicle(box_id, img,temp1,temp2,end_timeR,end_timeG):
    ...
    if temp1=="green" and temp2=="red" and datetime.now() < end_timeR:
        ...
        if ((iy > 547) and (iy < 719) and (ix > 282) and (ix < 659)) or ((iy > 127) and (iy < 215) and (ix > 721) and (ix < 872)):
    ...
    if  end_timeR - datetime.now() < datetime.now()-datetime.now()  :      
        ...
        cv2.putText(img,"site2 green: wait" , (500, 50), cv2.FONT_HERSHEY_SIMPLEX, font_size, font_color, font_thickness)
    else:
         cv2.putText(img,"site1 red: " + str(end_timeR-datetime.now()) , (900, 500), cv2.FONT_HERSHEY_SIMPLEX, font_size, font_color, font_thickness)
         cv2.putText(img,"site2 green: " + str(end_timeR-datetime.now()) , (500, 50), cv2.FONT_HERSHEY_SIMPLEX, font_size, font_color, font_thickness)
             
    if temp1=="red" and temp2=="green" and datetime.now() < end_timeG:
        if ((iy > 241) and (iy < 366) and (ix > 2) and (ix < 443)) or ((iy > 295) and (iy < 403) and (ix > 964) and (ix < 1278)):
    ...
    if  end_timeR - datetime.now() < datetime.now()-datetime.now() :  
        cv2.putText(img, "site1 green: " + str(end_timeG-datetime.now()) , (500, 500), cv2.FONT_HERSHEY_SIMPLEX, font_size, font_color, font_thickness)
        cv2.putText(img, "site2 red: " + str(end_timeG-datetime.now()) , (900, 50), cv2.FONT_HERSHEY_SIMPLEX, font_size, font_color, font_thickness)
    else:
         cv2.putText(img, "site1 green: wait", (500, 500), cv2.FONT_HERSHEY_SIMPLEX, font_size, font_color, font_thickness)
...
def realTime():
    ...
    while True:
        ...
        # Find the objects from the network outputq
        postProcess(outputs,img,temp1,temp2,side1vec,side2vec,end_timeR,end_timeG)

        # Draw the crossing lines
        #1-1
        cv2.line(img, (2,241), (443,305), (255, 0, 255), 2)
        cv2.line(img, (3,300), (350,366), (0, 0, 255), 2)
        #1-2
        ...
        cv2.putText(img, "Car:        " + str(side1_list[0]) + "     " + str(side2_list[0]) +"     " + str(side3_list[0]) +"     " , (20, 40), cv2.FONT_HERSHEY_SIMPLEX, font_size, font_color,
                    font_thickness)
        cv2.putText(img, "Motorbike:  " + str(side1_list[1]) + "     " + str(side2_list[1]) +"     " + str(side3_list[1]) +"     " , (20, 60), cv2.FONT_HERSHEY_SIMPLEX, font_size, font_color,
                    font_thickness)
        cv2.putText(img, "Bus:        " + str(side1_list[2]) + "     " + str(side2_list[2]) +"     " + str(side3_list[2]) +"     " , (20, 80), cv2.FONT_HERSHEY_SIMPLEX, font_size, font_color,
                    font_thickness)
        cv2.putText(img, "Truck:      " + str(side1_list[3]) + "     " + str(side2_list[3]) +"     " + str(side3_list[3]) +"     ", (20, 100), cv2.FONT_HERSHEY_SIMPLEX, font_size, font_color,
                    font_thickness)
